...
<div class="card">
    {#if !allCardsShown}
        ...
        <div class="btns">
            <button class="minecraft-btn" on:click={() => alert("sorry server error please click on the other button")}>No :/</button>
            <button class="minecraft-btn" on:click={() => {
                if (currentCard < prompts.length - 1) currentCard++
                else allCardsShown = true
            }}>Okay !</button>
        </div>
    {:else}
...
<style>
    ...
    .card {
        ...
        max-width: 700px;
        color: whitesmoke;
        filter: drop-shadow(0 0 2em #2ead0b);
    }
